...
wurl = "https://api.open-meteo.com/v1/forecast?latitude=" + str(latitude) + "&longitude=" + str(
    longitude) + "&hourly=temperature_2m,is_day&current_weather=true&timezone=auto"
...
wcode = wc_extract(
    str(json_extract(wjson, ["current_weather", "weathercode"])[0]))
...
if farenheight:
    temp = str(round((float(temp) * 9/5) + 32))
    print(wcode, temp+"°F")
else:
    print(wcode, temp+"°C")
